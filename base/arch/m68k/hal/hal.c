...
static unsigned long rtai_cpufreq_arg = MCF_CLK;
...
int rt_request_linux_irq (unsigned irq, void *handler, char *name, void *dev_id)
{
	...
	spin_unlock(&irq_controller[irq]->lock);
#endif
...
#define SAVE_REG \
	"move	#0x2700, %sr\n\t"			\
	"movel	(0xE1000000 + 0x10),%sp@-\n\t"		\
	"movel	(0xE1000000 + 0x8),%sp@-\n\t"		\
	"clrl	%sp@-\n\t"				\
	"movel	%d0,%sp@-\n\t"				\
	"movel	%d0,%sp@-\n\t"				\
	"subal	#(8*4), %sp\n\t"			\
...
void rt_free_timer (void)
{
	...
	hal_root_domain->irqs[RTAI_TIMER_LINUX_IRQ].acknowledge(RTAI_TIMER_LINUX_IRQ);
...
static int rtai_hirq_dispatcher (unsigned irq, struct pt_regs *regs)
{
	...
	else
	{
		...
		if (hal_root_domain->irqs[irq].acknowledge)
