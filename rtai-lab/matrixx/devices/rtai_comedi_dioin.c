...
extern int rtRegisterComediDioIn( int, void * );
extern struct {
	int ID ;
	void *comdev ;
...
#endif
{
	if(INFO->INIT) {
		...
		if ( rtRegisterComediDioIn( 1000*(IP[1]+1)+IP[0] , (void *)comdev ) ){
			exit_on_error();
		}
		index = IP[1];
	...
	if(INFO->OUTPUTS)  {
		...
		int i;
		struct DICOMDev *comdev;
		unsigned int bit;


		for ( i=0 ; i<N_DIOIN; i++ ){
			if ( ComediDioIn[i].ID == 1000*(IP[1]+1)+IP[0] )
				break;
		}

		comdev = (struct DICOMDev *) ComediDioIn[i].comdev;
